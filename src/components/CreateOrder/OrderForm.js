...
const OrderForm = ({handleOrder,productList,setProductList,productData,setProductData,totalWeight,totalWidth,totalHeight,totalLength,setTotalWeight,setTotalLength,setTotalHeight,setTotalWidth}) => {
  const handleProductList = () =>{
    if(productData?.name.length===0  || productData?.weight?.length===0 || productData?.height?.length===0 || productData?.length?.length===0 || productData?.productWidth?.length===0){
      ...
      setTotalWeight(totalWeight + Number(productData.weight));
      setTotalLength(totalLength + Number(productData.length))
      setTotalHeight(totalHeight+Number(productData.height))
      setTotalWidth(totalWidth + Number(productData.width))
  ...
  const removeProduct = (x) =>{
    setProductList(productList=> productList.filter(item=>item.productName !== x.productName))
  }
  return (
      ...
      <div>
        ...
        <div className="bg-white h-[390px] overflow-y-scroll px-16 py-6 my-3 rounded-md">
          ...
          <div className="my-6">
            ...
            <div className="my-3 bg-[#750e215f] py-2 px-3 gap-3 grid grid-cols-5 grid-flow-col ">
              <div>
            ...
            <div className='' >
            {
              productList?.map((product)=>(
                <div className="my-3 bg-[#9393935f] py-3 px-3 gap-3 grid grid-cols-5 grid-flow-col ">
              <div>
                <p className='font-[InterMedium] text-[.8rem]' >{product.name}</p>
              </div>
              ...
              <div>
                ...
                <div>
                <p onClick={()=>removeProduct(product)} className='font-[InterMedium] cursor-pointer text-red-300 w-[20px] text-[.8rem]' ><TrashIcon /> </p>
              </div>
            </div>
              ))
            }
            ...
            <div className="my-3 bg-[#0000005f] text-white py-3 px-3 gap-3 grid grid-cols-5 grid-flow-col ">
              <div>
                <p className='font-[InterMedium] text-[.8rem]' >Total</p>
              </div>
             
              <div>
                <p className='font-[InterMedium] text-[.8rem]' >{totalWeight} gm</p>
              </div>
                <div>
                <p className='font-[InterMedium] text-[.8rem]' >{totalWidth}cm</p>
              </div>
                <div>
                <p className='font-[InterMedium] text-[.8rem]' >{totalHeight}cm</p>
              </div>
                <div>
                <p className='font-[InterMedium] text-[.8rem]' >{totalLength}cm</p>
              </div>
                <div>
          ...
          <div className="flex items-center justify-end">
            <button onClick={handleOrder} className="bg-[#750E21] font-[InterMedium] text-[.8rem] rounded-md py-2 px-8 text-white ">
             Book
            </button>
          </div>
        </div>
